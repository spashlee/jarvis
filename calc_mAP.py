...
from ava_evaluation import object_detection_evaluation, action_detection_evaluation, standard_fields
...
def read_csv_det(csv_file, class_whitelist=None, capacity=0):
    """Loads boxes and class labels from a CSV file in the human detection format.
    ...
    for image_key in entries:
        ...
        for item in entry:
            score, action_id, y1, x1, y2, x2 = item
            if [y1, x1, y2, x2] in boxes[image_key]:
                continue
            boxes[image_key].append([y1, x1, y2, x2])
            labels[image_key].append(action_id)
            scores[image_key].append(score)
...
def run_evaluation(labelmap, groundtruth, detections, exclusions, logger):
    ...
    boxes, labels, scores = read_csv(detections, class_whitelist, 60)
    start = time.time()
    for image_key in boxes:
        if image_key in excluded_keys:
...
def run_det_evaluation(labelmap, groundtruth, detections, exclusions, logger):
    """Runs evaluations given input files.
    ...
    pascal_evaluator = object_detection_evaluation.ObjectDetectionEvaluator(
        categories, matching_iou_threshold=0.5)

    # Reads the ground truth data.
    boxes, labels, _ = read_csv_det(groundtruth, class_whitelist, 0)
    start = time.time()
    for image_key in boxes:
        if image_key in excluded_keys:
            logger.info(("Found excluded timestamp in ground truth: %s. "
        ...
        pascal_evaluator.add_single_ground_truth_image_info(
            image_key, {
                standard_fields.InputDataFields.groundtruth_boxes:
                    np.array(boxes[image_key], dtype=float),
                standard_fields.InputDataFields.groundtruth_classes:
                    np.ones(len(labels[image_key]), dtype=int),
                standard_fields.InputDataFields.groundtruth_difficult:
                    np.zeros(len(boxes[image_key]), dtype=bool)
            })
    print_time("convert groundtruth", start)
    # Reads detections data.
    boxes, labels, scores = read_csv_det(detections, class_whitelist, 100)
    start = time.time()
    for image_key in boxes:
        if image_key in excluded_keys:
            logger.info(("Found excluded timestamp in detections: %s. "
    ...
    start = time.time()
    metrics = pascal_evaluator.evaluate()
    print_time("run_det_evaluator", start)
    logger.info(pprint.pformat(metrics, indent=2))

    return metrics


def run_act_evaluation(labelmap, groundtruth, detections, exclusions, logger):
    """Runs evaluations given input files.
    ...
    pascal_evaluator = action_detection_evaluation.PascalDetectionEvaluator(
        categories, matching_iou_threshold=0.)
