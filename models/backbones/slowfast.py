...
class SlowFast(nn.Module):
    ...
    def SlowPath(self, input, Tc):
        ...
        x1 = self.slow_res1(x)
        x1 = torch.cat([x1, Tc[1]], dim=1)
        x2 = self.slow_res2(x1)
        x2 = torch.cat([x2, Tc[2]], dim=1)
        x3 = self.slow_res3(x2)
        x3 = torch.cat([x3, Tc[3]], dim=1)
        x4 = self.slow_res4(x3)
        return [x2, x3, x4]
    ...
    def FastPath(self, input):
        ...
        x1 = self.fast_res1(x)
        Tc2 = self.Tconv2(x1)
        x2 = self.fast_res2(x1)
        Tc3 = self.Tconv3(x2)
        x3 = self.fast_res3(x2)
        Tc4 = self.Tconv4(x3)
        x4 = self.fast_res4(x3)
        return [x2, x3, x4], [Tc1, Tc2, Tc3, Tc4]
