...
__all__ = ['vit_small_patch16_224', 'vit_base_patch16_224', 'vit_base_patch16_384', 'vit_large_patch16_224', 
           'vit_large_patch16_384', 'vit_large_patch16_512', 'vit_huge_patch16_224']
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        T, H, W = x.shape[2:]  # t,h,w
        x = x.flatten(2).transpose(1, 2)  # b,n,c
        B, _, C = x.shape
        # pos_embed=[8x14x14,384]->sin->[1,8x14x14,384]
        pos_embed = self.pos_embed

        if self.pos_embed.shape[1] != T * H * W:
            # pos_embed=[1 8*14*14 384]->[1 8*16*29 384]
            pos_embed = pos_embed.reshape(T, -1, C)
            pos_embed = interpolate_pos_embed_online(
                pos_embed, self.grid_size, [H, W], 0).reshape(1, -1, C)
